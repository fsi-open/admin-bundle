...
class CreateContext implements ContextInterface
{
    ...
    public function handleRequest(Request $request)
    {
        ...
        $this->dispatcher->dispatch(AdminEvents::CRUD_CREATE_CONTEXT_POST_CREATE, $event);
        if ($event->hasResponse()) {
            return $event->getResponse();
        }

        if ($request->isMethod('POST')) {
            $this->dispatcher->dispatch(AdminEvents::CRUD_CREATE_FORM_REQUEST_PRE_BIND, $event);
            if ($event->hasResponse()) {
                return $event->getResponse();
            }
            ...
            $this->dispatcher->dispatch(AdminEvents::CRUD_CREATE_FORM_REQUEST_POST_BIND, $event);
            if ($event->hasResponse()) {
                return $event->getResponse();
            }

            if ($this->element->getCreateForm()->isValid()) {
                $this->dispatcher->dispatch(AdminEvents::CRUD_CREATE_ENTITY_PRE_SAVE, $event);
                if ($event->hasResponse()) {
                    return $event->getResponse();
                }
                ...
                $this->dispatcher->dispatch(AdminEvents::CRUD_CREATE_ENTITY_POST_SAVE, $event);
                if ($event->hasResponse()) {
                    return $event->getResponse();
                }
        ...
        $this->dispatcher->dispatch(AdminEvents::CRUD_CREATE_RESPONSE_PRE_RENDER, $event);
        if ($event->hasResponse()) {
            return $event->getResponse();
        }
