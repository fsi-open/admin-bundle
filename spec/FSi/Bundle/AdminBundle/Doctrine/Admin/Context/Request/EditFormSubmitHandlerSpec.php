...
class EditFormSubmitHandlerSpec extends ObjectBehavior
{
    ...
    function it_throw_exception_for_non_list_event(ListEvent $listEvent, Request $request)
    {
        $this->shouldThrow(
                new RequestHandlerException(
                    "FSi\\Bundle\\AdminBundle\\Doctrine\\Admin\\Context\\Request\\EditFormSubmitHandler require FormEvent"
                )
    ...
    function it_do_nothing_on_non_POST_request(
        ...
        $eventDispatcher->dispatch(CRUDEvents::CRUD_EDIT_CONTEXT_POST_CREATE, $event)
            ->shouldBeCalled();
    ...
    function it_submit_form_on_POST_request(
        ...
        $eventDispatcher->dispatch(CRUDEvents::CRUD_EDIT_FORM_REQUEST_PRE_SUBMIT, $event)
            ->shouldBeCalled();

        $event->getForm()->willReturn($form);
        $form->submit($request)->shouldBeCalled();

        $eventDispatcher->dispatch(CRUDEvents::CRUD_EDIT_FORM_REQUEST_POST_SUBMIT, $event)
            ->shouldBeCalled();
