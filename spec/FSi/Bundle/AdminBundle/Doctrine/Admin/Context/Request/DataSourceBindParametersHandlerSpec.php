...
class DataSourceBindParametersHandlerSpec extends ObjectBehavior
{
    ...
    function it_bind_request_to_datasource_and_dispatch_events(
        ...
        $eventDispatcher->dispatch(CRUDEvents::CRUD_LIST_CONTEXT_POST_CREATE, $event)->shouldBeCalled();
        $eventDispatcher->dispatch(CRUDEvents::CRUD_LIST_DATASOURCE_REQUEST_PRE_BIND, $event)->shouldBeCalled();

        $event->getDataSource()->willReturn($dataSource);
        $dataSource->bindParameters($request)->shouldBecalled();

        $eventDispatcher->dispatch(CRUDEvents::CRUD_LIST_DATASOURCE_REQUEST_POST_BIND, $event)->shouldBeCalled();
    ...
    function it_return_response_from_context_post_create_event(
        ...
        $eventDispatcher->dispatch(CRUDEvents::CRUD_LIST_CONTEXT_POST_CREATE, $event)
    ...
    function it_return_response_from_pre_datasource_bind_parameters_event(
        ...
        $eventDispatcher->dispatch(CRUDEvents::CRUD_LIST_CONTEXT_POST_CREATE, $event)->shouldBeCalled();
        $eventDispatcher->dispatch(CRUDEvents::CRUD_LIST_DATASOURCE_REQUEST_PRE_BIND, $event)
            ->will(function() use ($event) {
                $event->hasResponse()->willReturn(true);
                $event->getResponse()->willReturn(new Response());
            });
    ...
    function it_return_response_from_post_datasource_bind_parameters_event(
        ...
        $eventDispatcher->dispatch(CRUDEvents::CRUD_LIST_DATASOURCE_REQUEST_POST_BIND, $event)
            ->will(function() use ($event) {
                $event->hasResponse()->willReturn(true);
                $event->getResponse()->willReturn(new Response());
            });
