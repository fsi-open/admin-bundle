...
class DataGridSetDataHandlerSpec extends ObjectBehavior
{
    ...
    function it_throw_exception_for_non_list_event(AdminEvent $event, Request $request)
    {
        $this->shouldThrow(
                new RequestHandlerException(
                    "FSi\\Bundle\\AdminBundle\\Doctrine\\Admin\\Context\\Read\\Request\\DataGridSetDataHandler require ListEvent"
                )
    ...
    function it_set_data_at_datagrid_and_dispatch_events(
        ...
        $eventDispatcher->dispatch(CRUDEvents::CRUD_LIST_DATAGRID_DATA_PRE_BIND, $event)->shouldBeCalled();
        ...
        $eventDispatcher->dispatch(CRUDEvents::CRUD_LIST_DATAGRID_DATA_POST_BIND, $event)->shouldBeCalled();
    ...
    function it_return_response_from_datagrid_pre_bind_data(
        ...
        $eventDispatcher->dispatch(CRUDEvents::CRUD_LIST_DATAGRID_DATA_PRE_BIND, $event)
    ...
    function it_return_response_from_datagrid_post_bind_data(
        ...
        $eventDispatcher->dispatch(CRUDEvents::CRUD_LIST_DATAGRID_DATA_POST_BIND, $event)
            ->will(function() use ($event) {
                $event->hasResponse()->willReturn(true);
                $event->getResponse()->willReturn(new Response());
            });
